...
def get_youtube_info_from_api(url):
    ...
    try:
        ...
        response = requests.get(api_url, timeout=45) # 45-second timeout
        response.raise_for_status() # Raises an error for bad responses (like 404 or 500)
        data = response.json()

        if not data.get("success"):
            print(f"API returned success=false for {url}")
            return None

        # --- Transform the API data into the structure our app expects ---
        ...
        for item in download_links.get("mp4", []):
            formats_list.append({
                ...
                "type": "video",
                "direct_url": item.get("url") # IMPORTANT: This is a direct download link
            })
        ...
        # Return a dictionary that matches the structure our frontend expects
        return {
        ...
        print("Error decoding JSON from NepCoder API")
        return None
...
def get_info_with_yt_dlp(url):
    ...
    # This is your 'get_video_info' function, renamed for clarity
    ...
    try:
        ...
        # Transform the yt-dlp data into our standard format
        return {
            'title': info.get('title', 'Untitled Video'),
            'thumbnail': info.get('thumbnail'),
            'formats': parse_yt_dlp_formats(info), # Use the parser below
            'original_url': url
        }
...
def parse_yt_dlp_formats(info):
    """This is your 'parse_formats' function, renamed for clarity."""
    formats_list = []
...
def fetch_info():
    ...
    # Both methods now return the same JSON structure, so we just return it
    return jsonify(info)
...
def download():
    ...
    # Your existing download code is perfect for this and requires no changes.
    ...
    except subprocess.CalledProcessError as e: print(f"!!!!!!!! YT-DLP STDERR (DOWNLOAD) !!!!!!!!\n{e.stderr}"); return jsonify({'error': 'Download failed. See server logs for details.'}), 500
    except subprocess.TimeoutExpired: return jsonify({'error': 'Download timed out.'}), 500
