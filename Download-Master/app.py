...
# --- SECTION 1: NEW - Logic for the External YouTube API ---
...
def get_youtube_info_from_api(url):
    ...
    try:
        ...
        # Process MP4 (video) links from the API
        ...
        # Process MP3 (audio) links from the API
...
# --- SECTION 2: Your Existing Logic for ALL OTHER Platforms ---

def get_info_with_yt_dlp(url):
    """Your original method, now used for non-YouTube sites."""
...
def parse_yt_dlp_formats(info):
    ...
    formats_list = []
    title = info.get('title', 'video'); sanitized_title = get_sanitized_filename(title)
    best_audio = next((f for f in reversed(info.get('formats', [])) if f.get('acodec') != 'none' and f.get('vcodec') == 'none'), None)
    if best_audio:
        filesize = best_audio.get('filesize') or best_audio.get('filesize_approx'); filesize_mb = f"~{filesize / (1024*1024):.2f} MB" if filesize else "N/A"
    ...
    video_formats = [f for f in info.get('formats', []) if f.get('vcodec') != 'none']; processed_labels = set()
    for f in reversed(video_formats):
        ...
        format_id = f"bestvideo[height<={height}]+bestaudio/best[height<={height}]"; filesize = f.get('filesize') or f.get('filesize_approx'); filesize_mb = f"~{filesize / (1024*1024):.2f} MB" if filesize else "N/A"
...
def get_yt_dlp_command(url):
    ...
    if os.path.exists(cookie_file_path): base_command.extend(['--cookies', cookie_file_path])
...
def convert_facebook_url(url):
    if 'facebook.com/share/v/' in url:
        ...
        if match:
            video_id = match.group(1); return f'https://www.facebook.com/watch/?v={video_id}'
    return url
def get_standard_label(height):
    if height >= 3240: return "4320p (8K)";    if height >= 1800: return "2160p (4K)";    if height >= 1260: return "1440p (2K)";    if height >= 900:  return "1080p (FHD)";    if height >= 600:  return "720p (HD)";    if height >= 420:  return "480p (SD)";    if height >= 300:  return "360p"
    return f"{height}p"
...
def fetch_info():
    """API endpoint that intelligently decides which download method to use."""
...
def download():
    ...
    url = request.args.get('url'); format_id = request.args.get('format_id'); filename = request.args.get('filename'); ext = request.args.get('ext')
    if not all([url, format_id, filename, ext]): return jsonify({'error': 'Missing required parameters for download.'}), 400
    unique_id = str(uuid.uuid4()); output_template = os.path.join(TMP_DIR, f"{unique_id}.%(ext)s"); final_filepath = os.path.join(TMP_DIR, f"{unique_id}.{ext}")
    command = get_yt_dlp_command(url); command.extend(['-o', output_template])
    if ext == 'mp3': command.extend(['-f', 'bestaudio/best', '--extract-audio', '--audio-format', 'mp3'])
    else: command.extend(['-f', format_id, '--merge-output-format', 'mp4'])
    try: subprocess.run(command, check=True, timeout=600)
    except Exception as e: print(f"Download Error: {e}"); return jsonify({'error': 'Download failed.'}), 500
    if not os.path.exists(final_filepath): return jsonify({'error': 'Downloaded file not found on server.'}), 500
    @after_this_request
    def cleanup(response):
        try: os.remove(final_filepath)
        except Exception as e: app.logger.error(f"Error cleaning up file: {e}")
        return response
