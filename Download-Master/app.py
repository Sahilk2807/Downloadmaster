...
# --- This is now the ONLY method for fetching video info ---
def get_yt_dlp_command(url):
    """Builds the base yt-dlp command for all platforms."""
...
def fetch_info():
    """API endpoint to fetch video info using only the internal yt-dlp."""
    data = request.get_json()
    url = data.get('url')
    if not url: return jsonify({'error': 'URL is required.'}), 400

    # The logic is now simplified: always use our own yt-dlp
    print(f"Using internal yt-dlp for: {url}")
...
def download():
    """API endpoint to download the selected format for ALL platforms."""
